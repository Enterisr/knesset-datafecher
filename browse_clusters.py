...
with st.sidebar:
    clusters_path = st.text_input('Clusters file', value=_ARGS.clusters)
    filter_text   = st.text_input('Filter (title / text)', '')
    show_noise    = st.checkbox('Show noise in 3D', value=False)

clusters, sorted_ids, params, coords3d, labels, metadata, cluster_names = load_clusters(clusters_path)
